...
docker exec database bash -c '
    # Backup original
    cp /var/lib/pgsql/13/data/pg_hba.conf /var/lib/pgsql/13/data/pg_hba.conf.bak
...
# Docker network - WRONG! Only allows 172.20.0.99
host    all             all             172.20.0.99/32          md5
EOF

    chown postgres:postgres /var/lib/pgsql/13/data/pg_hba.conf
    chmod 600 /var/lib/pgsql/13/data/pg_hba.conf

    # Reload PostgreSQL
    su - postgres -c "/usr/pgsql-13/bin/pg_ctl reload -D /var/lib/pgsql/13/data"
...
echo ""
echo "Hint: The backend IP is 172.20.0.11"
